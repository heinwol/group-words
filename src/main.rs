...
use std::{iter::once, fmt::{self, Formatter, Display, Debug}, hash::Hash, ops::Mul, };
...
impl Word {
    ...
    pub fn new(elts: &[WordLetter]) -> Self {
        Word { elements: Vec::from(elts) }.free_reduction()
    }
    ...
    pub fn to_string_with_separator(&self, sep: &str) -> String {
        let s: Vec<_> = self.elements.iter().map(
            |(chr, pow)| {
                if *pow != 1 { format!("{chr}^{pow}") }
                else         { format!("{chr}") }
            }
        ).collect();
        s.join(sep)
    ...
    pub fn inv(&self) -> Self {
        Self {
            elements: self.elements.iter().rev().map(
                |(chr, pow)| { (*chr, -*pow) }
            ).collect()
        }
    }
    ...
    pub fn symbols(&self) -> HashSet<char> {
        self.elements.iter().map(|(chr, pow)| *chr).collect::<HashSet<char>>()
    }
}
...
pub fn all_cyclic_permutations<T>(v: &Vec<T>) ->
...
pub fn maximum_word_partitioning(pcs: &HashSet<Word>, w: &Word) ->
    Vec<Word> {
        let literal_form = w.literal_form();
        if literal_form.len() < 2 {
            return literal_form.iter().map(
                |(chr, pow)| Word::new(
                    &Vec::from([(*chr, *pow)])
                )
            ).collect();
...
fn iterate_sth() {
    let vars = vec!['a', 'b'];
    let vars_word = vars.iter().map(
        |chr| Word::from_singleton(*chr)
    ).collect::<HashSet<Word>>();

    let letters_with_inverses = 
        (&vars_word | &inv_set(&vars_word))
            .into_iter().collect::<Vec<Word>>();
    ...
    let res = all_c6_single_relator_groups(
        multiprod.map(|w_vec| 
            multiply_many(&w_vec)
        ));
    println!("{:?}", res.len());
